 ...
 * 快捷通业务核心处理类
 * @Author jiangpeng
 * @CreateAt 2015/1/5 11:14
 */
public class KjtCore {
    ...
    static{
        ...
        try {
            sender.initCertWithKey(KjtConfig.pfxPath, KjtConfig.pfxKey);
            recipient.initCertWithKey(KjtConfig.pfxPath, KjtConfig.pfxKey);

            InputStream streamCert = new FileInputStream(KjtConfig.certPath);
    ...
    public static Map<String, String> buildRequestPara(Map<String, String> sParaTemp,String signType, String key,String inputCharset) throws Exception {
        ...
        if(signType==null || "".equals(signType))return sPara;
        // 生成签名结果
...
//        byte [] changString=prestr.getBytes("iso8859-1");
//        String mysign = sender.signMessage(new String(changString));
        String mysign = new String(Base64.encode(sender.signMessage(prestr.getBytes("UTF-8"))));
...
//                    value=new String(changString);
    ...
    /**
     * 获取响应结果，并根据key取得value
     * @author jiangpeng
     * @createAt 2015/1/6 12:57
     * @param content
     * @return
     */
    public static String getResultValue(String content,String key){
        //快捷通响应串基本类似且不存在特殊字符
